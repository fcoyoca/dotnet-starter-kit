...
import {
  createPatientDocumentType,
  deletePatientDocumentType,
  listPatientDocumentTypes,
  updatePatientDocumentType,
  type PatientDocumentTypeDto,
  type CreatePatientDocumentTypeInput,
  type UpdatePatientDocumentTypeInput,
...
type EditorState =
  | { mode: "closed" }
  | { mode: "create" }
  | { mode: "edit"; type: PatientDocumentTypeDto }
  | { mode: "delete"; type: PatientDocumentTypeDto };

export function PatientDocumentTypesPage() {
  ...
  const query = useQuery({
    queryKey: ["administration", "patient-document-types", { search: debouncedSearch, pageNumber, pageSize: PAGE_SIZE }],
    queryFn: () => listPatientDocumentTypes({ search: debouncedSearch || undefined, pageNumber, pageSize: PAGE_SIZE }),
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <EntityPageHeader
        icon={FileText}
        title="Patient Document Types"
        total={data?.totalCount ?? null}
        unit="type"
        description="Categories used to classify uploaded patient documents."
      >
        <Button
      ...
      {query.isLoading && items.length === 0 ? (
        ...
        <EntityEmpty
          ...
          title={searchActive ? "No patient document types found" : "No patient document types yet"}
          body={
            searchActive
              ? `Nothing matches "${debouncedSearch}". Try a different term or clear the search.`
              : "Add your first patient document type to start classifying patient documents."
          }
          action={
            searchActive ? (
      ...
      <PatientDocumentTypeEditorDialog state={editor} onClose={() => setEditor({ mode: "closed" })} />
      <DeletePatientDocumentTypeDialog state={editor} onClose={() => setEditor({ mode: "closed" })} />
    </div>
  );
}

function MobileCard({ type, onEdit }: { type: PatientDocumentTypeDto; onEdit: () => void }) {
  return (
    <EntityMobileCard
      href="#"
      onClick={(e) => {
      ...
      aria-label={`Edit patient document type ${type.name}`}
    >
...
function PatientDocumentTypeEditorDialog({ state, onClose }: { state: EditorState; onClose: () => void }) {
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["administration", "patient-document-types"] });

  const createMutation = useMutation({
    mutationFn: (input: CreatePatientDocumentTypeInput) => createPatientDocumentType(input),
    onSuccess: () => {
      toast.success("Patient document type created");
      invalidate();
      onClose();
    },
  ...
  const updateMutation = useMutation({
    mutationFn: (input: UpdatePatientDocumentTypeInput) => updatePatientDocumentType(input),
    onSuccess: () => {
      toast.success("Patient document type updated");
      invalidate();
      onClose();
    },
    ...
    <Dialog open={isOpen} onOpenChange={(o) => (!o ? onClose() : undefined)}>
      <DialogContent className="!max-w-md">
        <form onSubmit={onSubmit}>
          <DialogHeader>
            <DialogTitle>{type ? "Edit patient document type" : "Add a patient document type"}</DialogTitle>
            <DialogDescription>
              {type ? `Update details for ${type.name}.` : "Add a patient document type to your organization."}
            </DialogDescription>
          </DialogHeader>

          <DialogBody className="space-y-5">
            <Field id="doc-type-name" label="Name" required>
              <Input
                id="doc-type-name"
                value={form.name}
            ...
            {type && (
              <div className="flex items-center justify-between rounded-lg border border-[var(--color-border)] px-3 py-2.5">
                ...
                <Switch
                  ...
                  aria-label="Patient document type active"
                />
              </div>
            )}
          </DialogBody>
...
function DeletePatientDocumentTypeDialog({ state, onClose }: { state: EditorState; onClose: () => void }) {
  const isOpen = state.mode === "delete";
  const type = state.mode === "delete" ? state.type : undefined;
  const queryClient = useQueryClient();

  const deleteMutation = useMutation({
    mutationFn: (id: string) => deletePatientDocumentType(id),
    onSuccess: () => {
      toast.success("Patient document type deleted");
      queryClient.invalidateQueries({ queryKey: ["administration", "patient-document-types"] });
      onClose();
    },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(o) => (!o ? onClose() : undefined)}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="text-[var(--color-destructive)]">Delete patient document type</DialogTitle>
          <DialogDescription>
            This removes{" "}
