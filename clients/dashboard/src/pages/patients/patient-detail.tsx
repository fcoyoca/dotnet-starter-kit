    ...
    <div className="pb-12">
      ...
      {patientQuery.isLoading ? (
        ...
        <>
          <PatientHero
            patient={patient}
            onEdit={() => setDialog({ mode: "edit-demographics" })}
            onToggleStatus={() => setDialog({ mode: "toggle-status" })}
            onDelete={() => setDialog({ mode: "delete" })}
          />

          <div className="grid grid-cols-1 gap-5 lg:grid-cols-[300px_1fr]">
            {/* Left: at-a-glance + sensitive + audit */}
              ...
              <EntityDetailSection
                ...
                action={
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setDialog({ mode: "edit-demographics" })}
                    className="gap-1.5"
                  >
              ...
              <EntityDetailSection
                ...
                action={
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setDialog({ mode: "edit-phi" })}
                    className="gap-1.5"
                  >
                    <Pencil className="h-3.5 w-3.5" />
                    Edit
              ...
              <EntityDetailSection
                ...
                action={
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setDialog({ mode: "edit-flags" })}
                    className="gap-1.5"
                  >
                    <Pencil className="h-3.5 w-3.5" />
                    Edit
            ...
            {/* Right: contact, kin, employment, insurance */}
              ...
              <EntityDetailSection
                ...
                action={
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setDialog({ mode: "edit-next-of-kin" })}
                    className="gap-1.5"
                  >
                    <Pencil className="h-3.5 w-3.5" />
                    Edit
              ...
              <EntityDetailSection
                ...
                action={
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setDialog({ mode: "edit-employment" })}
                    className="gap-1.5"
                  >
                    <Pencil className="h-3.5 w-3.5" />
                    Edit
                ...
                <EntityDetailSection
                  ...
                  action={
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setDialog({ mode: "edit-guardian" })}
                      className="gap-1.5"
                    >
                      <Pencil className="h-3.5 w-3.5" />
                      Edit
              ...
              <EntityDetailSection
                ...
                action={
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setDialog({ mode: "edit-insurance" })}
                    className="gap-1.5"
                  >
                    <Pencil className="h-3.5 w-3.5" />
                    Edit
          ...
          <DemographicsDialog
            open={dialog.mode === "edit-demographics"}
            patient={patient}
            onClose={() => setDialog({ mode: "closed" })}
          />
          <ContactDialog
            open={dialog.mode === "edit-contact"}
            patient={patient}
            onClose={() => setDialog({ mode: "closed" })}
          />
          <PhiDialog
            open={dialog.mode === "edit-phi"}
            patient={patient}
            onClose={() => setDialog({ mode: "closed" })}
          />
          <EmploymentDialog
            open={dialog.mode === "edit-employment"}
            patient={patient}
            onClose={() => setDialog({ mode: "closed" })}
          />
          <GuardianDialog
            open={dialog.mode === "edit-guardian"}
            patient={patient}
            onClose={() => setDialog({ mode: "closed" })}
          />
          <NextOfKinDialog
            open={dialog.mode === "edit-next-of-kin"}
            patient={patient}
            onClose={() => setDialog({ mode: "closed" })}
          />
          <InsuranceDialog
            open={dialog.mode === "edit-insurance"}
            patient={patient}
            onClose={() => setDialog({ mode: "closed" })}
          />
          <FlagsDialog
            open={dialog.mode === "edit-flags"}
            patient={patient}
            onClose={() => setDialog({ mode: "closed" })}
          />
          <ToggleStatusDialog
...
function PatientHero({
  ...
  return (
    <EntityDetailHero
      avatar={<EntityDetailAvatar name={display} icon={Stethoscope} />}
      title={display}
      badges={
        <>
          {patient.isActive ? (
          ...
          {patient.demographics.isMinor && (
            <EntityStatusBadge tone="info">Minor</EntityStatusBadge>
          )}
        </>
      }
      subtitle={
        <span className="inline-flex flex-wrap items-center gap-x-2 gap-y-1">
          <code className="rounded bg-[var(--color-muted)] px-1.5 py-0.5 font-mono text-[11px] tracking-tight text-[var(--color-foreground)]">
            {patient.patientCode}
          </code>
          ...
          <span>
            {formatDate(patient.demographics.dateOfBirth)} ({age} yrs)
          </span>
        </span>
      }
      actions={
        <>
          ...
          <Button variant="outline" size="sm" onClick={onToggleStatus} className="gap-1.5">
            {patient.isActive ? (
              <>
      ...
      stats={
        <>
          <EntityDetailStat icon={Cake} value={age} label="years old" tone="primary" />
          ...
          {patient.nextVisitDate && (
            <EntityDetailStat
              icon={CalendarCheck}
              value={formatDate(patient.nextVisitDate)}
              label="next visit"
              tone="success"
            />
          )}
      ...
      meta={
        <>
          {patient.contact.email && (
            <EntityDetailMeta icon={MapPin} hideOnMobile>
              {patient.contact.email}
            </EntityDetailMeta>
          )}
          ...
          <EntityDetailMeta icon={Info} hideOnMobile>
            Created {formatRelative(patient.createdAtUtc)}
          </EntityDetailMeta>
          {patient.updatedAtUtc && (
            <EntityDetailMeta icon={Info} hideOnTablet>
              Updated {formatRelative(patient.updatedAtUtc)}
            </EntityDetailMeta>
          )}
        </>
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] px-8 py-16 text-center">
      <div className="mb-5 grid size-16 place-items-center rounded-2xl bg-[oklch(from_var(--color-primary)_l_c_h_/_0.08)]">
        <Stethoscope className="size-7 text-[var(--color-primary)]" />
      </div>
      <h3 className="mb-1.5 text-[17px] font-semibold text-[var(--color-foreground)]">Patient not found</h3>
