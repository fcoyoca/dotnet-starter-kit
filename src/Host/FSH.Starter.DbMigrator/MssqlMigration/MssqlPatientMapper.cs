...
internal static class MssqlPatientMapper
{
    ...
    /// Uses ROW_NUMBER() pagination (compatible with SQL Server 2008 compat level 100).
    /// </summary>
    public static string BuildSelectQuery(int offset, int batchSize) =>
        ...
        SELECT
            ...
            pEmployerPhone,
            pParentGuardianSSN,
            pParentGuardianFirstName,
            pParentGuardianLastName,
            pParentGuardianMiddleInitial,
            pParentGuardianDOB,
            pParentGuardianSex,
            pParentGuardianMaritalStatus,
            pParentGuardianAddress1,
            pParentGuardianAddress2,
            pParentGuardianCity,
            pParentGuardianState,
            pParentGuardianZip,
            pParentGuardianPhoneNumber,
            pParentGuardianCellPhoneNumber,
            pParentGuardianEmployerName,
            pParentGuardianEmployerAddress1,
            pParentGuardianEmployerAddress2,
            pParentGuardianEmployerCity,
            pParentGuardianEmployerState,
            pParentGuardianEmployerZip,
            pRelativeFirstName,
            pRelativeLastName,
            pRelativePhone,
            pRelativeRelation,
        ...
        FROM (
            SELECT
                ...
                p.pMI,
                CONVERT(datetime, CONVERT(NVARCHAR(256), DECRYPTBYKEY(p.pDOB)))   AS pDOB,
                ...
                CONVERT(datetime, CONVERT(NVARCHAR(256), DECRYPTBYKEY(p.pParentGuardianDOB))) AS pParentGuardianDOB,
                ...
                ROW_NUMBER() OVER (ORDER BY p.pUniqueID) AS _RowNum
            FROM dbo.Patients p
            WHERE p.pMergedToPatientID IS NULL OR p.pMergedToPatientID = '0' OR p.pMergedToPatientID = ''
        ) AS _Paged
        WHERE _RowNum > {offset} AND _RowNum <= {offset + batchSize}
        """;
    ...
    public static CreatePatientCommand Map(SqlDataReader r)
    {
        ...
        var gender = GetStringOrEmpty(r, "pSex");
        if (gender.Length == 0) gender = "UN"; // legacy records with no sex value

        var isMinor = GetBoolOrDefault(r, "pMinor", false);
        var guardianFirstName = GetString(r, "pParentGuardianFirstName");
        // Legacy records marked minor but with no guardian data — treat as non-minor.
        if (isMinor && guardianFirstName is null) isMinor = false;
